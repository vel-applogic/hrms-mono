...
export class CandidateUpdateDocumentsUc extends BaseCandidateUc implements IUseCase<Params, OperationStatusResponseType> {
  ...
  private async syncOfferLetters(params: Params, tx: Prisma.TransactionClient): Promise<void> {
    ...
    for (const media of params.dto.offerLetters) {
      if (media.key?.startsWith('temp/')) {
        await this.createAndLinkMedia({ media, candidateId: params.id, organisationId: params.currentUser.organisationId, type: CandidateMediaType.offerLetter, tx });
      } else if (media.id) {
  ...
  private async syncOtherDocuments(params: Params, tx: Prisma.TransactionClient): Promise<void> {
    ...
    for (const media of params.dto.otherDocuments) {
      if (media.key?.startsWith('temp/')) {
        await this.createAndLinkMedia({ media, candidateId: params.id, organisationId: params.currentUser.organisationId, type: CandidateMediaType.otherDocuments, tx });
      } else if (media.id) {
