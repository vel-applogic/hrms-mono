...
import { OrganisationDeleteDialog } from './container/organisation-delete.dialog';
import { OrganisationUpsertDrawer } from './container/organisation-upsert.drawer';
import { OrganisationDataTableClient } from './organisation.datatable';
...
export const OrganisationData = ({ data, searchParams }: Props) => {
  ...
  const [editingOrganisation, setEditingOrganisation] = useState<OrganisationResponseType | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deletingOrganisation, setDeletingOrganisation] = useState<OrganisationResponseType | null>(null);
  ...
  const handleEdit = (organisation: OrganisationResponseType) => {
    setEditingOrganisation(organisation);
    setDrawerOpen(true);
  };

  const handleDelete = (organisation: OrganisationResponseType) => {
  ...
  return (
    <div className='flex h-full flex-col gap-4'>
      <div className='center-container flex items-center justify-between'>
        <span className='text-xl font-medium tracking-tight text-foreground'>Organisations</span>
      ...
      <div className='center-container flex flex-1 flex-col min-h-0'>
        <OrganisationDataTableClient data={data} sort={sort} onEdit={handleEdit} onDelete={handleDelete} />
      </div>

      <OrganisationUpsertDrawer open={drawerOpen} onOpenChange={setDrawerOpen} organisation={editingOrganisation} onSuccess={() => refresh()} />

      <OrganisationDeleteDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} organisation={deletingOrganisation} onSuccess={() => refresh()} />
    </div>
  );
};
