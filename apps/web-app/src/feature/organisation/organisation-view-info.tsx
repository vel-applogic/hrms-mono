...
export function OrganisationViewInfo({ organisationId }: Props) {
  ...
  return (
    <>
      <div className='mb-6 flex items-center justify-between'>
        <h2 className='text-lg font-medium'>Organisation Info</h2>
      ...
      <div className='grid gap-6 md:grid-cols-2'>
        ...
        <div className='flex flex-col gap-2'>
          ...
          {organisation.logo ? (
            <img src={organisation.logo.urlFull} alt='Logo' className='h-16 w-16 rounded-md border border-border object-contain' />
          ) : (
        ...
        <div className='flex flex-col gap-2'>
          ...
          <p className='text-sm font-medium'>{new Date(organisation.createdAt).toLocaleDateString()}</p>
        </div>
        <div className='flex flex-col gap-2'>
          <Label className='text-muted-foreground'>Updated at</Label>
          <p className='text-sm font-medium'>{new Date(organisation.updatedAt).toLocaleDateString()}</p>
        </div>
      </div>
        ...
        {organisation.address ? (
          <div className='grid gap-4 md:grid-cols-2'>
            <div className='flex flex-col gap-1'>
              ...
              <p className='text-sm font-medium'>{organisation.address.addressLine1}</p>
            </div>
            {organisation.address.addressLine2 && (
              <div className='flex flex-col gap-1'>
                <Label className='text-muted-foreground'>Address line 2</Label>
                <p className='text-sm font-medium'>{organisation.address.addressLine2}</p>
              </div>
            )}
            <div className='flex flex-col gap-1'>
              <Label className='text-muted-foreground'>City</Label>
              <p className='text-sm font-medium'>{organisation.address.city}</p>
            </div>
            <div className='flex flex-col gap-1'>
              <Label className='text-muted-foreground'>State</Label>
              <p className='text-sm font-medium'>{organisation.address.state}</p>
            </div>
            <div className='flex flex-col gap-1'>
              <Label className='text-muted-foreground'>Postal code</Label>
              <p className='text-sm font-medium'>{organisation.address.postalCode}</p>
            </div>
            <div className='flex flex-col gap-1'>
              <Label className='text-muted-foreground'>Country</Label>
              <p className='text-sm font-medium'>{organisation.address.country.name}</p>
            </div>
          </div>
        ) : (
