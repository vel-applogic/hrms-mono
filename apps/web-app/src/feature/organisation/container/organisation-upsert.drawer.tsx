...
import {
  ...
  ContactResponseType,
  OrganisationDocumentResponseType,
...
import { createOrganisation, getOrganisationById, listCountries, listCurrencies, updateOrganisation } from '@/lib/action/organisation.actions';
...
const FORM_ID = 'organisation-upsert-form';

export function OrganisationUpsertDrawer({ open, onOpenChange, organisation, onSuccess }: Props) {
  ...
  const [existingDocuments, setExistingDocuments] = useState<OrganisationDocumentResponseType[]>([]);
  ...
  useEffect(() => {
    if (open) {
      ...
      if (organisation) {
        updateForm.reset({ name: organisation.name, currencyId: organisation.currency.id, settings: DEFAULT_SETTINGS, contacts: [] });
        setFetchingDetail(true);

        getOrganisationById(organisation.id).then((result) => {
  ...
  }, [open, organisation, createForm, updateForm]);
  ...
  const handleRemoveExistingDocument = (doc: OrganisationDocumentResponseType) => {
  ...
  return (
    <Drawer
      ...
      dismissible={false}
      title={isEditing ? 'Edit organisation' : 'New organisation'}
      footer={
      ...
      {isEditing ? (
        ...
        <form id={FORM_ID} onSubmit={createForm.handleSubmit(handleCreateSubmit)} className='flex flex-col gap-6 p-6'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <p className='text-xs text-muted-foreground'>This user will be invited as admin of the organisation</p>
          </div>
