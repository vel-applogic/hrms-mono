  ...
  return (
    <div className='hidden h-[52px] items-center gap-2.5 lg:flex'>
      {tabs.map((tab) => {
        ...
        return (
          <Link key={tab.href} href={tab.href} className='group relative flex h-[52px] items-center px-3 pb-2'>
            <span className={`text-sm font-bold tracking-widest transition-colors group-hover:text-foreground ${isActive ? 'text-foreground' : 'text-muted-foreground'}`}>{tab.label}</span>
