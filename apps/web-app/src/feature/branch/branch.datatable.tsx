...
import type { BranchResponseType, PaginatedResponseType } from '@repo/dto';
...
interface Props {
  ...
  onEdit: (branch: BranchResponseType) => void;
  onDelete: (branch: BranchResponseType) => void;
  onRefresh: () => void;
}
...
export const BranchDataTableClient = (props: Props) => {
  const colDefs = useMemo<ColDef<BranchResponseType>[]>(() => {
    return [
      {
        headerName: 'Id',
        field: 'id',
      ...
      },
      {
        ...
        sortable: isSortable('name', BranchSortableColumns),
      ...
      },
      {
        ...
        sortable: isSortable('createdAt', BranchSortableColumns),
        comparator: DummySort,
      ...
      },
      {
        ...
        sortable: isSortable('updatedAt', BranchSortableColumns),
        comparator: DummySort,
        cellRenderer: DateTimeRenderer,
      },
      {
        ...
        cellRenderer: ActionsIconCellRenderer<BranchResponseType>,
        cellRendererParams: {
          options: actionOptions,
        } satisfies Partial<ActionsIconCellRendererParams<BranchResponseType>>,
      },
    ] satisfies ColDef<BranchResponseType>[];
  }, [props.sort.sKey, props.sort.sVal]);

  const onActionClick = useCallback(
    (action: string, data: BranchResponseType) => {
      switch (action) {
        case 'Edit':
          props.onEdit(data);
          break;
  ...
  return (
    <DataTableSimple<BranchResponseType>
      colDefs={colDefs}
