...
import { ChapterDetailResponseType, ChapterListResponseType, PaginatedResponseType, SearchParamsType } from '@repo/dto';
...
import { ChapterDataTableClient } from './chapter.datatable';
import { ChapterSummaryPoints } from './chapter-summary-points';
import { ChapterDeleteDialog } from './container/chapter-delete.dialog';
import { ChapterUpsertDrawer } from './container/chapter-upsert.drawer';
import { getChapterById } from '@/lib/action/chapter.actions';
...
export const ChapterData = ({ data, searchParams }: Props) => {
  ...
  const [editingChapter, setEditingChapter] = useState<ChapterDetailResponseType | null>(null);
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [deletingChapter, setDeletingChapter] = useState<ChapterListResponseType | null>(null);
  ...
  const handleEdit = async (chapter: ChapterListResponseType) => {
  ...
  const handleDelete = (chapter: ChapterListResponseType) => {
    ...
    <div className='flex h-full flex-col gap-4 pt-4'>
      ...
      {viewMode === 'table' ? (
        <div className='center-container flex flex-1 flex-col min-h-0 pb-4'>
          <ChapterDataTableClient data={data} sort={sort} onEdit={handleEdit} onDelete={handleDelete} />
        </div>
      ) : (
        <>
          ...
          <div className='center-container'>
            <div className='mx-auto flex w-full max-w-[1000px] flex-col gap-3'>
              {data.results.map((chapter) => (
                <div key={chapter.id} className='flex flex-col gap-4 rounded-xl bg-card px-6 pb-4 pt-6'>
                  <div className='flex flex-col gap-4'>
                    <div className='flex items-center justify-between'>
                      <span className='text-base font-medium text-white'>{chapter.title}</span>
                      <span className='text-sm text-muted-foreground'>
                        {new Date(chapter.createdAt).toLocaleDateString()}
                      </span>
                    </div>
                    {chapter.description && (
                      <p className='text-base leading-snug text-secondary-foreground'>{chapter.description}</p>
                    )}
                    {chapter.summaryPoints && chapter.summaryPoints.length > 0 && (
                      <ChapterSummaryPoints summaryPoints={chapter.summaryPoints} />
                    )}
                  </div>
      ...
      <ChapterUpsertDrawer open={drawerOpen} onOpenChange={setDrawerOpen} chapter={editingChapter} onSuccess={handleDrawerSuccess} />

      <ChapterDeleteDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} chapter={deletingChapter} onSuccess={handleDeleteSuccess} />
    </div>
  );
};
