...
function CompensationCard({
  ...
  const fromLabel = formatDate(compensation.effectiveFrom);
  const tillLabel = formatDate(compensation.effectiveTill);

  return (
    <div className={cn('relative rounded-md border border-muted-foreground/30 p-4', compensation.isActive ? 'bg-white dark:bg-white/5' : 'bg-muted/30')}>
      {fromLabel && (
